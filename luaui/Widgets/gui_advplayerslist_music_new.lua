...
local function SetSoundStreamVolume(newVolume)
	Spring.SetSoundStreamVolume(newVolume)
	Spring.Echo("Set to " .. newVolume)
end

local function getFastFadeSpeed()
	-- return 1.5 * 0.33
	return getSlowFadeSpeed()
end
...
local function setVolume(fadeLevel)
	Spring.Echo("SetSoundStreamVolume 2")
	SetSoundStreamVolume(getMusicVolume() * math.max(math.min(fadeLevel, 100), 0) * 0.01)
end
...
local function updateFade()
	if fadeDirection then
		...
		if fadeDirection < 0 and fadelevel <= 0 then
			fadeDirection = nil
			Spring.Echo("Completed fadeout!")
			if fadeOutSkipTrack then
				PlayNewTrack()
			else
				Spring.Echo("No skip requested, not starting a new track yet")
				Spring.StopSoundStream()
			end
		elseif fadeDirection > 0 and fadelevel >= 100 then
			fadeDirection = nil
			Spring.Echo("Completed fade in!")
		end
	end
end
...
function PlayNewTrack(paused)
	...
	if (not gameOver) and Spring.GetGameFrame() > 1 then
		fadelevel = 0
		Spring.Echo("Beginning fade in!")
		fadeDirection = 1
	else
		-- fade in only when game is in progress
		fadelevel = 100
		fadeDirection = nil
		Spring.Echo("Playing song with no fade in!")
	end
	currentTrack = nil
	...
	if currentTrack then
		...
		Spring.Echo("Playing " .. currentTrack)
		if fadeDirection then
			setVolume(fadelevel)
		else
			Spring.Echo("SetSoundStreamVolume 2")
			SetSoundStreamVolume(musicVolume)
		end
	else
		Spring.Echo("No new track to play! List: " .. currentTrackListString)
	end
		...
		if not gameOver then
			if playedTime > 0 and totalTime > 0 then -- music is playing
				if not fadeDirection then
					Spring.Echo("SetSoundStreamVolume 1")
					SetSoundStreamVolume(musicVolume)
					if (totalTime < playedTime+11.1) then
						Spring.Echo("Beginning fade out! (1)")
						fadeDirection = -1
					...
					or (( (currentTrackListString == "warLow" or currentTrackListString == "warHigh") and warMeter <= warLowLevel * 0.2 ) and interruptionEnabled) then -- War music is playing, but it has been quite peaceful recently. Let's switch to peace music at 20% of WarLow threshold 
						Spring.Echo("Beginning fade out! (2)")
						fadeDirection = -1
						fadeOutSkipTrack = true
					end
				end
